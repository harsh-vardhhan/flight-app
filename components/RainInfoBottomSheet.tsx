...
const RainInfoBottomSheet: React.FC<RainInfoBottomSheetProps> = ({
  ...
  const precipitationData: PrecipitationData = {
    'Hanoi': { 'Jan': 1.9, 'Feb': 2.2, 'Mar': 4.6, 'Apr': 6.7, 'May': 12.2, 'Jun': 14.4, 'Jul': 16.3, 'Aug': 17.2, 'Sep': 12.6, 'Oct': 8.0, 'Nov': 4.2, 'Dec': 2.0 },
    'Ho Chi Minh City': { 'Jan': 0.9, 'Feb': 0.6, 'Mar': 1.6, 'Apr': 4.3, 'May': 11.7, 'Jun': 15.9, 'Jul': 16.7, 'Aug': 15.7, 'Sep': 16.6, 'Oct': 16.5, 'Nov': 8.4, 'Dec': 2.9 },
    'Da Nang': { 'Jan': 4.5, 'Feb': 1.8, 'Mar': 2.0, 'Apr': 3.2, 'May': 7.2, 'Jun': 7.2, 'Jul': 7.1, 'Aug': 10.8, 'Sep': 15.5, 'Oct': 18.3, 'Nov': 13.8, 'Dec': 9.7 },
    'Phu Quoc': { 'Jan': 2.0, 'Feb': 2.3, 'Mar': 5.2, 'Apr': 9.7, 'May': 15.8, 'Jun': 19.6, 'Jul': 21.3, 'Aug': 21.6, 'Sep': 20.6, 'Oct': 19.4, 'Nov': 11.0, 'Dec': 3.9 },
  };
  ...
  const getBarColor = (isCurrentMonth: boolean): string => isCurrentMonth ? CURRENT_MONTH_FILL_COLOR : BASE_BAR_COLOR;
  ...
  const currentMonthIndex = useMemo(() => {
    try {
      ...
      return !isNaN(dateObj.getTime()) ? dateObj.getMonth() : new Date().getMonth();
    } catch {
  ...
  const rainRisk = useMemo(() => {
    if (rain_probability < 25) return { level: 'Low', color: isDarkMode ? '#66BB6A' : '#2ecc71' };
    if (rain_probability <= 50) return { level: 'Moderate', color: isDarkMode ? '#4A90E2' : '#3498db' };
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderRelease: (_, gestureState) => {
        ...
        if (gestureState.dy > 0 && (gestureState.vy > 0.3 || finalPosition > BOTTOM_SHEET_HEIGHT / 3)) {
  ...
  const openBottomSheet = () => {
    ...
    }).start(() => { lastBaseY.current = 0; });
  };

  const closeBottomSheet = (fromValue?: number) => {
    panY.setValue(0);
    if (typeof fromValue === 'number') { animatedBaseY.setValue(fromValue); }
    Animated.timing(animatedBaseY, {
  ...
  useEffect(() => {
    if (visible) {
      if (lastBaseY.current >= SCREEN_HEIGHT) { openBottomSheet(); }
    } else {
      if (lastBaseY.current < SCREEN_HEIGHT) { closeBottomSheet(); }
    }
  }, [visible]);
  ...
  return (
    <View style={styles.container} pointerEvents={visible ? 'auto' : 'none'}>
      ...
      >
        ...
        <Text style={styles.bottomSheetTitle}>{displayDestination} Rain Statistics</Text>
        ...
        <View style={styles.destinationContainer}>
          ...
          {displayDestination === closestDestination && (
            <View style={styles.currentRainContainer}>
              <Text style={styles.rainProbabilityText}>Rain Probability: <Text style={styles.highlightText}>{rain_probability}%</Text></Text>
              <View style={[styles.riskIndicator, { backgroundColor: rainRisk.color }]}>
        ...
        <View style={styles.chartContainer}>
          {months.map((month, index) => {
            ...
            const barHeightPercent = maxRainDays > 0 ? (rainDays / maxRainDays) * 100 : 0;
              ...
              <View key={month} style={styles.monthColumn}>
                ...
                >
                  ...
                  <View style={styles.labelContainer}>
                    <Text style={styles.rainDaysLabel}>{rainDays.toFixed(1)}</Text>
                    <Text style={styles.emojiLabel}>{getRainEmoji(rainDays, maxRainDays)}</Text>
                  </View>
                  {/* Bar SECOND */}
                  <View
                    style={[
                      styles.bar,
                      { height: `${barHeightPercent}%`, backgroundColor: getBarColor(isCurrentMonth) },
                      isCurrentMonth && styles.currentMonthBarHighlight,
                    ]}
                  />
                </View>
                <Text style={[styles.monthLabel, isCurrentMonth && styles.currentMonthLabelHighlight]}>
                  {month}
                </Text>
              </View>
            );
