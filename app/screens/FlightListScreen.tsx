...
const FlightListScreen = () => {
  ...
  const handleSelectFlight = useCallback((flight: Flight, direction: 'Outbound' | 'Return') => {
    dispatch({
      type: 'SELECT_FLIGHT',
  ...
  const renderFooter = () => state.loadingMore ? (
    <View style={styles.footerLoader}>
      <ActivityIndicator size="small" color={isDarkMode ? '#3399ff' : '#0066cc'} />
  ...
  const renderEmptyList = () => (state.loading && state.page === 1) ? null : (
  ...
  if (state.loading && state.page === 1) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={{ color: isDarkMode ? '#ddd' : '#333' }}>Loading flights...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <FilterTabs
        ...
        routes={routes}
        onOriginSelect={(origin) => dispatch({ type: 'SET_ORIGIN', payload: origin })}
        onDestinationSelect={(destination) => dispatch({ type: 'SET_DESTINATION', payload: destination })}
        onBaggageOptionChange={(option) => dispatch({ type: 'SET_BAGGAGE_OPTION', payload: option })}
        onBaggageWeightChange={(weight) => dispatch({ type: 'SET_BAGGAGE_WEIGHT', payload: weight })}
        onDrySeasonToggle={(value) => dispatch({ type: 'SET_DRY_SEASON', payload: value })}
        onPriceFilterToggle={(value) => dispatch({ type: 'SET_PRICE_FILTER', payload: value })}
      />
      ...
      <View style={styles.totalCountContainer}>
        ...
        >
          <Ionicons
            ...
            size={16}
            color={state.sortByDate ? (isDarkMode ? '#00cc00' : '#00a000') : (isDarkMode ? '#aaa' : '#555')}
          />
          <Text style={[
